	...
	switch(RG1mode->ItemIndex){
		case(0):
			if(Ecitizen->ItemIndex==-1 || CBedu->ItemIndex==-1 || CBdoc_edu->ItemIndex==-1){
				Application->MessageBoxW(String("Выберите существующие значения из выпадающих списков!").w_str(),String("Ошибка").w_str(),MB_OK+MB_ICONWARNING);
				break;
			}
			DM->Tenr->Last();
			{
			...
			if(RG2sex->ItemIndex==0) DM->Tenrsex->AsString="муж";
			else DM->Tenrsex->AsString="жен";
		...
		case(1):
			...
			switch(CBedu->ItemIndex){
				case(0): DM->Tenredu->AsString="cреднее общее"; break;
				case(1): DM->Tenredu->AsString="среднее профессиональное"; break;
				case(2): DM->Tenredu->AsString="начальное профессиональное"; break;
			}
			switch(CBdoc_edu->ItemIndex){
				case(0): DM->Tenrdoc_edu->AsString="аттестат"; break;
				case(1): DM->Tenrdoc_edu->AsString="диплом"; break;
			}  */
			if(CBhonors->Checked==true) DM->Tenrhonors->AsString="1";
			else DM->Tenrhonors->AsString="0";
			DM->Tenr->Post();
			...
			CBdoc_edu->ItemIndex=-1;;
			CBhonors->Checked=false;
			//BitBtn4Click(Sender); Выполнение метода переводит таблицу на последнюю запись
...
void __fastcall TFinputedit::BitBtn2Click(TObject *Sender)
{
	for(int i=0;i<5;i++){
		try{
			if(Application->MessageBoxW(String("Вы действительно хотите удалить данную запись?").w_str(),String("Подтверждение действия").w_str(),MB_YESNO+MB_ICONQUESTION)==IDYES){
...
void __fastcall TFinputedit::DBG1enrCellClick(TColumn *Column)
{
	if(RG1mode->ItemIndex==1){
			...
			if(DM->Tenrsex->AsString=="муж") RG2sex->ItemIndex=0;
...
void __fastcall TFinputedit::BitBtn5Click(TObject *Sender)
{
	...
	while(!DM->Tadm->Eof){
		...
		try {
			...
			if (value>=100 || DM->Tadmhonors->AsString=="1"){
				cntEmtBud++;
				DM->Temtbud_com->AsString="бюджет";
			}
			else{
				cntEmtCom++;
				DM->Temtbud_com->AsString="коммерция";
			}
	...
	//ShowMessage("Операция выполнена! Обработано записей: "+cntEnr);
}
//---------------------------------------------------------------------------
...
void __fastcall TFinputedit::BitBtn7Click(TObject *Sender)
{
	if(DM->Temt->Filtered){
		...
		BitBtn7->Caption="Включить фильтры";
	}
	else{
		DM->Temt->Filtered=true;
		BitBtn7->Caption="Выключить фильтры";
	}
}
//---------------------------------------------------------------------------
...
/*	if(DM->Tadm->Filtered){
		DM->Tadm->Filtered=false;
	  //	DM->Tadm->Filter="";
		BitBtn8->Caption="Показать";
	}else{
	  //	DM->Tadm->Filter="last_name="+LEslastnadm->Text;
		DM->Tadm->Filtered=true;
		BitBtn8->Caption="Сбросить";
	}
	*/
}
//---------------------------------------------------------------------------
...
void __fastcall TFinputedit::Button2Click(TObject *Sender)
{
	if(DM->Tenr->Filtered){
		...
		Button2->Caption="Включить фильтры";
	}
	else{
		DM->Tenr->Filtered=true;
		Button2->Caption="Выключить фильтры";
	}
}
//---------------------------------------------------------------------------
