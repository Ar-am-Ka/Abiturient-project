...
{
	if(DM->Titavailable->AsInteger==0) ShowMessage("Товара нет в наличии");
	else {
		CSpinEdit1->Value=1;
		LE4->Text=DM->Titname->AsString;
		DBGit->Enabled=false;
		int a=(DM->Titprice->AsInteger)*(CSpinEdit1->Value);
		Label2->Caption="Цена: "+IntToStr(a)+" руб.";
	}
}
//---------------------------------------------------------------------------
...
void __fastcall TFsale::Button8Click(TObject *Sender)
{
	...
	//отчет
	frxReport1->Print();
	if(Application->MessageBoxW(String("Печать чека...        Экспортировать чек в файл PDF?").w_str(),String("АРГУС").w_str(),MB_YESNO+MB_ICONQUESTION)==IDYES){
